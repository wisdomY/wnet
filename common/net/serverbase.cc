...
ServerBase::ServerBase(evpp::EventLoop* loop,
                       ...
                       uint32_t thread_num)
    : loop_(loop), streamer_(streamer), adapter_(adapter)
    , tcp_server_(loop, listen_addr, name, thread_num), stopped_(false)
{
    tcp_server_.SetConnectionCallback(std::bind(&ServerBase::ConnectionCallback, this, std::placeholders::_1));
    tcp_server_.SetMessageCallback(std::bind(&ServerBase::OnMessage, this, std::placeholders::_1, std::placeholders::_2));
}
...
{
    tcp_server_.Stop([this]() { 
        stopped_ = true;
        stopCond_.notify_one();
    });
    std::unique_lock<std::mutex> _lk(stopMutex_);
    stopCond_.wait(_lk, [this] {
        return stopped_;
    });
}

void ServerBase::ConnectionCallback(const evpp::TCPConnPtr& conn)
{
