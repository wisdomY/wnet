...
* date   :  2018-09-06
* readme:   
* 若json中的key为var，则变量名必须定义为m_var, 解析的时候只有最外层为开始用
* PRASE_JSON_OBJECT_BEGIN与PRASE_JSON_OBJECT_END， 
* 其余内层对象都使用PRASE_JSON_SUB_OBJECT_BEGIN与PRASE_JSON_SUB_OBJECT_END
*************************************************************************************************/
...
#define PRASE_JSON_OBJECT_BEGIN \
    ...
    { \
        ...
        if (jsonDoc.HasParseError()) { \
            LOG_ERROR << "Json Parse error:" << jsonDoc.GetParseError() \
...
//解析double，json传输最好不要使用double类型，容易失真
